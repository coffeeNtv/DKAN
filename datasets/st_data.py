...
class BaselineDataset(torch.utils.data.Dataset):
    ...
    def get_text(self, type: str, emb_dim_text: int, text_model: str):
        """Load gene description data of a sample.

        Args:
            type (str): name of dataset

        Returns:
            pandas.DataFrame: return gene description. 
        """
        path = f"{self.data_dir}/ST-text/{type}_ncbi_biobert.pt"
        df = torch.load(path, weights_only=False)

        return df["gene_features"]


class STDataset(BaselineDataset):
    """Dataset to load ST data for DKAN
    """
    def __init__(self, 
                 mode: str, 
                 fold: int=0, 
        ...
        if 'emb_dim_text' in kwargs:
            self.emb_dim_text = kwargs['emb_dim_text']
        if 'text_model' in kwargs:
        ...
        if 'text_model' in kwargs and 'text_model' in kwargs:
            self.text = self.get_text(self.type, self.emb_dim_text, self.text_model)
        else:
            self.text = None
        ...
        self.id2name = dict(enumerate(self.names)) # patient name
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            name = self.id2name[i] # i is the index, id2name is the patient
            ...
            neighbors = torch.load(self.data_dir + f"/{self.neighbor_dir}/{name}.pt", weights_only=False)[idx]
        else:
            i = index
            name = self.id2name[i]
            ...
            neighbors = torch.load(self.data_dir +  f"/{self.neighbor_dir}/{name}.pt", weights_only=False)
        
        wsi = torch.load(self.data_dir +  f"/{self.gt_dir}/{name}.pt", weights_only=False)
        ...
        if self.mode == 'train':
            return patches, exps, pid, sid, wsi, position, neighbors, mask_tb, self.text
        elif self.mode == 'inference':
            return patches, sid, wsi, position, neighbors, mask_tb, self.text
        else:
            return patches, exps, sid, wsi, position, name, neighbors, mask_tb, self.text
